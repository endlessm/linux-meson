...
int meson_ioctl_msync(struct drm_device *dev, void *data, struct drm_file *file)
{
	...
	struct meson_drm_gem_object *gem_meson = NULL;
	...
	if (gem_obj->is_no_cma) {
		gem_meson = to_meson_drm_gem_obj(gem_obj);
...
out:
	...
	if (gem_obj->is_no_cma)
		return PTR_ERR_OR_ZERO(gem_meson);
	else
